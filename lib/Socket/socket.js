...
const makeSocket = (config) => {
    ...
    if (url.protocol === 'wss' && ((_a = authState?.creds) === null || _a === void 0 ? void 0 : _a.routingInfo)) {
    ...
    const noise = (0, Utils_1.makeNoiseHandler)({
        ...
        logger,
        routingInfo: (_b = authState?.creds) === null || _b === void 0 ? void 0 : _b.routingInfo
    });
    ...
    const logout = async (msg) => {
        var _a;
        const jid = (_a = authState.creds.me)?.id;
        if (jid) {
            await sendNode({
                tag: 'iq',
    ...
    const requestPairingCode = async (phoneNumber, pairKey = "YUKICODE") => {
        ...
        const pairingNonce = Math.floor(Date.now() / 1000).toString();
        ...
        await sendNode({
            ...
            content: [{
                ...
                content: [
                    ...
                    { tag: 'link_code_pairing_nonce', attrs: {}, content: pairingNonce }
                ]
            }]
        });

        ev.emit('connection.update', { connection: 'pairing', method: 'link_code', qr: undefined });
        const PAIR_TIMEOUT_MS = 20000;
        let pairTimer = setTimeout(() => {
            logger.warn('pairing via 8-digit timed out, forcing reconnect');
            end(new boom_1.Boom('Pairing timeout', { statusCode: Types_1.DisconnectReason.timedOut }));
        }, PAIR_TIMEOUT_MS);

        ws.on('CB:iq,,pair-success', () => clearTimeout(pairTimer));
        ws.on('CB:success', () => clearTimeout(pairTimer));
        ws.on('close', () => clearTimeout(pairTimer));
        ws.on('error', () => clearTimeout(pairTimer));
    ...
    ws.on('CB:failure', (node) => {
        ...
        logger.warn({ reason }, 'failure received, backing off before reconnect');
        setTimeout(() => end(new boom_1.Boom('Connection Failure', { statusCode: reason, data: node.attrs })), 5000);
    });
    ...
    ev.on('creds.update', update => {
        ...
        if (creds.me?.name !== name) {
            ...
            sendNode({ tag: 'presence', attrs: { name } })
                .catch(err => logger.warn({ trace: err.stack }, 'error in sending presence update on name change'));
        }
